...
export class TelegramBot {
    ...
    private registerCommands() {
        ...
        this.bot.command("register_domain", async (ctx) => {
            ...
            if (!domain) return ctx.reply("Specify the domain: /register_domain example.com");
            try {
        ...
        this.bot.command("ns", async (ctx) => {
            const [, domain] = ctx.message!.text.split(/\s+/);
            if (!domain) return ctx.reply("Specify the domain: /ns example.com");
            try {
                const zone = await this.cfClient.getZoneByName(domain);
                if (!zone) return ctx.reply("Zone not found or not active.");
                const ns = zone.name_servers || zone.nameservers || [];
        ...
        this.bot.command("dns_list", async (ctx) => {
            const [, domain] = ctx.message!.text.split(/\s+/);
            if (!domain) return ctx.reply("Specify the domain: /dns_list example.com");
            try {
                const zone = await this.cfClient.getZoneByName(domain);
